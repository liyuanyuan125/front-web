...
// 同步
export async function syncData(query, type) {
    const res = await get(`/theater/movies/sync/${query}`, { type });
    return res;
}
// 刷新
export async function reda(query) {
    const res = await get('/theater/movies', query);
    return res;
}
/**
 ...
 * @param status 新的分类
 */
export async function updateStatus(id, categoryCode) {
 ...
 */
export async function updateSpecialId(id, specialId) {
 ...
 */
export async function updateControlStatus(id, controlStatus) {
    const res = await put(`/theater/movies/${id}`, { controlStatus });
    return res;
}
export async function getIdDetal(id) {
