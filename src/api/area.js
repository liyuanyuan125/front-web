 ...
 * @param pid 父区域ID
 */
export async function getSubList(pid = 0) {
    const { data } = await get('/basis/districts', { parentIds: pid, pageSize: 888888 });
    const tlist = (data.items || []).map((it) => {
        return {
            ...it,
...
const isAllZero = (list) => (list || []).every(it => it === 0);
/**
 * 判断一组 ids 是否是合法的区域组合（正确的包含关系）
 * @param ids 区域组合，null 与空数组，返回 true
 */
export async function isValidArea(ids) {
    ...
    for (let i = 0, len = checkIds.length; i < len - 1; i++) {
        ...
        const foundItem = list.find((it) => it.id === aid);
