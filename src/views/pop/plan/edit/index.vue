    ...
    <Form :model="form" ref="dataform" label-position="left" :rules="rule" :label-width="100" class="edit-input">
      ...
      <div class="flex-box" >
        <FormItem  label="广告片规格">
            <span class="edit-item-text">{{detaultList.specification}}s</span>
          <!-- <span class="edit-item-text">{{specification}}s</span> -->
        </FormItem>
        <FormItem  label="选择客户">
            <span class="edit-item-text">{{detaultList.customerName}}</span>
          <!-- <span class="edit-item-text">{{customerName}}</span> -->
        </FormItem>
      </div>

      <!-- 投放排期 -->
      <h3 class="layout-title">投放排期</h3>
      <FormItem label="定向类型" class="form-item-type item-top">
      ...
      <div v-else key="edited" class="checkd">
        <div class="flex-box">
          <FormItem  label="选择年份" prop="year">
            <DatePicker type="year" v-model="form.year" @on-change="diaries()" placeholder="请选择年份"></DatePicker>
          </FormItem>
          <FormItem label="选择档期" prop="calendarId">
            <Select :disabled="!form.year" v-model="form.calendarId" filterable clearable>
      ...
      <FormItem label="投放类型" class=" item-top select-adv-type">
        <span :class="['put', putType== item.key ? 'put-active' : '']" 
        v-for="(item, index) in directionTypeList" v-if='index != 0' @click="putActive(item.key)" :key="item.key">
      ...
      <!-- 投放定向 按单部影片 -->
      <div v-else class="single-wrap item-top">
...
export default class Main extends ViewBase {
  ...
  async list() {
    try {
      ...
      this.form.beginDate = formatYell(item.beginDate, '-')
      this.form.endDate = formatYell(item.endDate, '-')
      this.form.year =  item.calendarYear
      // 投放定向类型
