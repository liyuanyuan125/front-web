<template>
  <div class="home-bg">
    ...
    <div class="text-rows">
      <Row>
        ...
        <Col :span="12">
          ...
          <!-- <p v-if="status == 3"><label>订单折扣</label> {{items.discount}} </p> -->
        </Col>
      </Row>
    </div>
    ...
    <div class="text-rows">
        <div v-if="status == 9">
            <p class="flex-box"><label>地域偏好</label><em><i v-for="item in areaList">{{item}}</i></em></p>
        </div>
        <div v-else>
        ...
        <div class="flex-box film-list">
            ...
            <div class="flex-box">
                <div class="item" v-for="item in defaultData.movieList">
                    ...
                    <div class="film-type-items"><i v-for="it in item.types">{{queryFilmList(it)}}<em> / </em></i></div>
                </div>
            </div>
        </div>
    </div>
    <div class="btnCenter btn-footer" v-if="status == 1">
      ...
      <Button type="primary" class="button-ok edit-btn" @click="$router.push({name: 'pop-plan-edit'})" >编辑</Button>
...
export default class PlanDefault extends ViewBase {
    ...
    async list() {
        try {
            ...
            const areas = data.item.deliveryGroups.filter( (item: any) => item.tagTypeCode == 'DISTRICT_AREA')
