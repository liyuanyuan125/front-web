      ...
      <div class="model-bg">
          ...
          <ul class="ul-lists">
              <li v-for="item in list" :key="item.id">
                 <span class="col-first span-first"><img :src="item.logo" class="logo" /><em>{{item.brandName}}</em></span>
                 <span class="col-second" v-for="ite in tradeStatus" :key="ite.key" v-if="ite.key == item.tradeCode">{{ite.text}}</span>
                 <span class="col-third" v-for="it in brandStatus" :key="it.key" v-if="it.key == item.status">{{it.text}}</span>
                 <span v-if="item.status == 15" class="col-fourth">
                     <img src="./assets/add-icon.png" width="20" class="cursor" @click="$router.push({name: 'brand-moredetail', params: {id: item.id, brandId: item.brandId}})" />
                 </span>
              </li>
          </ul>
      ...
      <Modal v-model="visibleBrand" title="添加品牌" width="700" class="modal-dlg">
        <Form ref="form" :model="form" :rules="rule" :label-width="90" class="edit-input">
            <FormItem label="品牌" prop="brandId" >
                <Select v-model="form.brandId"  clearable filterable remote :loading="loading" class="select-dlg"
                :remote-method="remoteMethod" @on-clear="brandList = []" style="width: 400px" placeholder="请输入品牌模糊查询">
...
export default class Main extends ViewBase {
    ...
    async tableList() {
      try {
        const { data: { items, totalCount, brandRelationStatus, tradeCodeStatus}} = await brandList({
          ...this.pageList
        })
        this.list = items || []
    ...
    async remoteMethod(query: any) {
      try {
        if (query) {
          this.loading = true
          const { data: {items} } = await selectBrand({
            name: query,
            pageIndex: 1,
            pageSize: 400,
          })
