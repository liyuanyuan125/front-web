 ...
 */
export function prettyQuery(data, defValMap = {}) {
...
/// buildUrl
const rUrlParams = /^([^?]*)(?:\?)?(.*)$/;
const decode = (s) => s != null ? decodeURIComponent(s).trim() : null;
const obj2Param = (obj) => {
...
const param2Obj = (str) => {
    return str.split('&').reduce((ret, it) => {
        const s = it.trim();
        if (s) {
 ...
 * @param defVal 参数默认值
 */
export function urlParam(name, defVal = null) {
